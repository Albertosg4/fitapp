...
export default function CalendarioMes({ horarios, onSeleccionarDia, marcadores = {} }: Props) {
  const hoy = new Date()
  ...
  const getHorariosDelDia = (d: Date): HorarioSocio[] => {
    const diaSemanaIdx = getDiaSemanaLunesPrimero(d)
    return horarios.filter(h => h.dia_semana === diaSemanaIdx)
  }

  const seleccionar = (d: Date) => {
    const fecha = formatLocalDate(d)
    setFechaSeleccionada(fecha)
    onSeleccionarDia(fecha, getHorariosDelDia(d))
  }
        ...
        {semana.map((d, i) => {
          ...
          return (
            <button key={i} onClick={() => seleccionar(d)}
              ...
              <div style={{ width: '5px', height: '5px', borderRadius: '50%', background: seleccionado ? '#0f0f0f' : marcador === 'reservada' ? '#c8f542' : tieneHorarios ? 'rgba(255,255,255,0.2)' : 'transparent' }} />
            </button>
          )
        })}
      </div>
