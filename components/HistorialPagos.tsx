  ...
  if (loading) {
    return (
      <div style={{ display: 'flex', justifyContent: 'center', padding: '20px' }}>
        <div style={{ width: '20px', height: '20px', borderRadius: '50%', border: '2px solid #c8f542', borderTopColor: 'transparent', animation: 'spin 0.8s linear infinite' }} />
      </div>
    )
  }
    ...
    return (
      <div>
        <p style={{ fontSize: '12px', color: '#888', marginBottom: '12px' }}>
          {pagos.length} pagos · Total: {total.toFixed(2)}€
        </p>
        <table style={{ width: '100%', fontSize: '13px', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.07)', textAlign: 'left' }}>
              <th style={{ paddingBottom: '8px', color: '#888', fontWeight: '500', paddingRight: '12px' }}>Fecha</th>
              <th style={{ paddingBottom: '8px', color: '#888', fontWeight: '500', paddingRight: '12px' }}>Tipo</th>
              <th style={{ paddingBottom: '8px', color: '#888', fontWeight: '500', paddingRight: '12px' }}>Importe</th>
              <th style={{ paddingBottom: '8px', color: '#888', fontWeight: '500', paddingRight: '12px' }}>Método</th>
              <th style={{ paddingBottom: '8px', color: '#888', fontWeight: '500' }}>Estado</th>
            </tr>
          </thead>
          <tbody>
            {pagos.map(p => {
              ...
              return (
                <tr key={p.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                  <td style={{ padding: '8px 12px 8px 0', color: '#aaa' }}>
                    {new Date(p.fecha_pago).toLocaleDateString('es-ES')}
                  </td>
                  <td style={{ padding: '8px 12px 8px 0', fontWeight: '600', color: '#f0f0f0', textTransform: 'capitalize' }}>
                    {p.tipo_membresia}
                  </td>
                  <td style={{ padding: '8px 12px 8px 0', color: '#c8f542', fontWeight: '700' }}>
          ...
          return (
            <div key={p.id} style={{ background: '#1e1e1e', border: `1px solid ${p.estado === 'pendiente' ? 'rgba(255,184,77,0.2)' : 'rgba(255,255,255,0.06)'}`, borderRadius: '12px', padding: '14px 16px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <div>
                  <div style={{ fontSize: '15px', fontWeight: '700', color: '#f0f0f0', textTransform: 'capitalize' }}>
                    {p.tipo_membresia} · {p.meses} {p.meses === 1 ? 'mes' : 'meses'}
                  </div>
                  <div style={{ fontSize: '12px', color: '#888', marginTop: '3px' }}>
                    {new Date(p.fecha_pago).toLocaleDateString('es-ES', { day: '2-digit', month: 'long', year: 'numeric' })}
                  </div>
              ...
              {p.estado === 'pendiente' && (
                <div style={{ marginTop: '8px', padding: '6px 10px', background: 'rgba(255,184,77,0.08)', borderRadius: '8px', fontSize: '12px', color: '#ffb84d' }}>
