...
interface Props {
  userId: string        // UUID del socio a mostrar
  limit?: number        // Cuántos registros mostrar (default: 20)
  compact?: boolean     // Vista compacta para el panel admin
}
...
export default function HistorialAsistencia({ userId, limit = 20, compact = false }: Props) {
  ...
  // Vista compacta para admin (tabla densa)
  if (compact) {
    return (
      <div>
        ...
        {total > limit && (
          <p className="text-xs text-gray-400 mt-2 text-right">
            Mostrando los últimos {limit} de {total}
          </p>
        )}
      </div>
    )
  ...
  // Vista completa para socio (cards)
  return (
    <div>
      {/* Resumen */}
      ...
      <div className="space-y-2">
        {historial.map((row) => (
          <div
            key={row.id}
            className="flex items-center justify-between bg-white border border-gray-100 rounded-xl px-4 py-3 hover:border-blue-200 transition-colors"
          >
      ...
      {total > limit && (
        <p className="text-xs text-gray-400 mt-4 text-center">
          Mostrando los últimos {limit} registros de {total}
        </p>
      )}
    </div>
  )
